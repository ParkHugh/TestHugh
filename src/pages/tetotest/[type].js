// pages/tetotest/result/[type].js

import { useRouter } from "next/router";
import resultImages from "../../../src/tests/tetotest/resultImages";
import resultDescriptions from "../../../src/tests/tetotest/resultDescriptions";

export default function TetoTestResultPage() {
  const router = useRouter();
  const { type } = router.query;

  // 아직 라우터가 준비 안된 상태 대응 (빌드시)
  if (!type) return null;
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="bg-white rounded-xl shadow p-8">
          ...
          <button onClick={() => router.push('/tetotest')} className="text-blue-600 underline">
            테스트 하러가기
          </button>
        </div>
      </div>
