...
// 색상 변수
const headerBg = 'bg-white';            // 헤더(기본 유지)
const mainBg = 'bg-[#fcf8ee]';          // 본문(연한 아이보리)
const footerBg = 'bg-[#fcf8ee]';        // 푸터(본문과 맞춤)
...
export default function MainPage() {
    ...
    ${test.id === 'sociopathtest' ? 'text-red-400 drop-shadow' : test.id === 'romantictest' ? 'text-pink-500' : 'text-emerald-700'}
  `}>
                                    {test.title}
                                </h2>
                                {test.id === 'tetotest' && (
  ...
  `}>
            ...
            <footer className={`w-full border-t ${footerBg} py-6 mt-10 text-center text-orange-300 text-sm`}>
                <div className="flex justify-center space-x-4">
                    {/* About만 메뉴에서 접근 가능 / Contact, Privacy는 푸터에만 */}
