// src/pages/adhdtest/result/[type].js

import { useRouter } from 'next/router';
import Head from 'next/head';
import Link from 'next/link';
import results from '@/tests/adhdtest/result';
...
export default function AdhdTestResultPage() {
  const router = useRouter();
  const { type } = router.query;

  const result = getResultByType(type);
  const resultUrl = `https://test-hugh.co.kr/adhdtest/result/${type || result.id}`;
  ...
  const handleShare = () => {
    ...
    if (navigator.share) {
      navigator.share({
        title: `성인 ADHD 테스트 | ${result.type} | Test 休`,
        text: "나의 ADHD 잠재력, 너도 확인해봐! 🌈🧠",
        url: resultUrl,
      }).catch(() => {});
    } else {
  ...
  return (
    <>
      <Head>
        <title>성인 ADHD 테스트 | {result.type} | Test 休</title>
        <meta name="description" content={`${result.type} 유형 해설 및 ADHD 특성 설명!`} />
        <meta property="og:title" content={`성인 ADHD 테스트 | ${result.type} | Test 休`} />
        <meta property="og:description" content={`${result.type} 유형 해설 및 ADHD 특성 설명!`} />
        <meta property="og:image" content={resultImgUrl} />
        <meta property="og:url" content={resultUrl} />
      </Head>

      <div className="min-h-screen bg-gradient-to-br from-pink-200 via-yellow-100 via-green-200 via-blue-200 to-fuchsia-200 flex flex-col items-center justify-center px-4 py-8">
        <div className="flex flex-col items-center w-full">
          ...
          />
          <div className="mb-3">
            <span className="inline-block bg-gradient-to-r from-pink-500 via-yellow-400 to-blue-500 text-white text-xs px-3 py-1 rounded-full shadow font-semibold tracking-wide mb-2">
              {result.type}
            </span>
          </div>
          ...
          {/* message 필드가 있으면 */}
          {result.message && (
            <div className="bg-gradient-to-r from-yellow-100 via-pink-100 to-blue-100 rounded-xl shadow-inner px-5 py-4 mx-auto max-w-lg mb-6 border-l-2 border-fuchsia-300 text-fuchsia-600 text-base font-semibold">
